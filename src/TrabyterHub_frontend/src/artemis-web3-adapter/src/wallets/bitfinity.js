...
export const bitfinity = window?.ic
    ? window?.ic?.bitfinityWallet && 'InfinityWallet!'
        ? {
              readyState: 'Installed',
              connectWallet: async function (
                  connectObj = {whitelist: [], host: ''},
              ) {
                  var isconn = await window.ic.bitfinityWallet.isConnected();
                  if (!isconn) {
                      let s = await window.ic.bitfinityWallet.requestConnect(
                          connectObj,
                      );
                  }
                  ...
                  this.createActor = async function (t1) {
                      return window.ic.bitfinityWallet.createActor(t1);
                  };
                  this.batchTransactions = async function (t1) {
                      return window.ic.bitfinityWallet.batchTransactions(t1);
                  };
